...
Late <- Late %>% 
  mutate(
    Period = case_when(
      Week %in% c(1,2,3,4) ~ "Before Fine",
      Week %in% c(17,18,19,20) ~ "Fine Removed",
      TRUE ~ "Fine Applied"
    )
  )
Late$Treatment <- Late$Treatment %>% as.factor()
Late$Period <- Late$Period %>% as.factor()
...
Null Hypotheses = Mean of Control Group = Mean of Fine Group.
Alt Hypotheses = Average number of late children for one group.
...
Null Hypotheses = Before Fine = Fine Applied = Removed Fine
Alt Hypotheses = Average number of late children differ for one group.

Hypotheses about the interaction of Treatment and Period.

#_____________________________________________________________

H_0: \text{The effect of the Treatment on the average Period picked up does not depend on the Period of the study.

H_a: \text{The effect of the [fill in the blank] on the average [fill in the blank] picked up changes during at least one [fill in the blank] of the study.
...
```{r}
#For children the two factors would be: Period, treatment.
#For the two-way ANOVA, we will use the aov() function.
#The formula will be NumberofLateChildren ~ Period * Treatment

result1 <- aov(NumberofLateChildren ~ Treatment + Period + Treatment:Period, data = Late)
summary(result1) %>% pander()

Late <- Late %>% 
  mutate(NumberofLateChildren = NumberofLateChildren %>% as.numeric())

xyplot(NumberofLateChildren ~ Treatment, data = Late, type = c("p", "a"))
xyplot(NumberofLateChildren ~ Period, data = Late, type = c("p", "a"))
xyplot(NumberofLateChildren ~ Treatment, data = Late, type = c("p", "a"), groups = Period)

summary_table <- Late %>%
  group_by(Treatment, Period) %>%
  summarise(mean_response = mean(NumberofLateChildren, na.rm = TRUE), .groups = "drop")

pander(summary_table)

sum <- Late %>%
  group_by(Treatment, Period) %>%
  summarise(mean_response = sum(NumberofLateChildren, na.rm = TRUE), .groups = "drop")
